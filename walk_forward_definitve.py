...
# Proporción train+val vs OOS
TRAIN_VAL_RATIO = 0.70   # 70% para folds, 30% para OOS final

def get_folds_and_oos(data: pd.DataFrame, ticker: str) -> tuple[list, str, str, float]:
    """
    Calcula folds y OOS dinámicamente según el historial real del ticker.

    - El 70% inicial se divide en folds expanding window de ~2 años de validación
    - El 30% final es el OOS intocable

    Returns:
        folds    : lista de dicts con label, train_end, val_start, val_end
        oos_start: fecha inicio OOS
        oos_end  : fecha fin OOS
    """
    all_dates  = data.index
    n          = len(all_dates)
    cutoff_idx = int(n * TRAIN_VAL_RATIO)

    oos_start = all_dates[cutoff_idx].strftime("%Y-%m-%d")
    oos_end   = all_dates[-1].strftime("%Y-%m-%d")

    # Periodo train+val: desde el inicio hasta el corte 70%
    tv_dates = all_dates[:cutoff_idx]
    tv_years = (tv_dates[-1] - tv_dates[0]).days / 365.25

    # Número de folds: ~2 años de validación cada uno, mínimo 2
    val_years  = 2.0
    n_folds    = max(2, int(tv_years / val_years) - 1)
    # Construir folds expanding window
    folds = []
    for i in range(n_folds):
        # Val: los últimos (i+1)*val_years del periodo train+val
        val_end_idx   = cutoff_idx - 1
        val_start_idx = cutoff_idx - int((i + 1) * val_years * 252)
        val_start_idx = max(val_start_idx, int(0.3 * cutoff_idx))  # mínimo 30% de datos de train
        ...
        train_end = all_dates[val_start_idx - 1].strftime("%Y-%m-%d")

        folds.append({
            "label":      f"Fold {n_folds - i}",
            "train_end":  train_end,
            "val_start":  val_start,
            "val_end":    val_end,
        })

    folds = list(reversed(folds))  # ordenar cronológicamente

    print(f"  Historial: {all_dates[0].date()} → {all_dates[-1].date()} "
          f"({tv_years + (1-TRAIN_VAL_RATIO)*tv_years/TRAIN_VAL_RATIO:.1f} años)")
    print(f"  Corte 70/30: train+val hasta {oos_start} | OOS desde {oos_start}")
    print(f"  Folds generados: {len(folds)}")
    for f in folds:
        print(f"    {f['label']}: val {f['val_start']} → {f['val_end']}")

    return folds, oos_start, oos_end, val_years


PARAM_GRID = {
    "momentum_lookback": [ 189, 252,315 ],   # 3m, 6m, 9m, 12m  "momentum_lookback": [63, 126, 189, 252],
    "momentum_skip":     [0, 21, 42],            # sin skip, 1m, 2m
    "alpha_min":         [0.0, 0.05, 0.10, 0.15],      # defensividad
}
...
def download_data(ticker: str) -> tuple[pd.DataFrame, pd.Series]:
    """
    Descarga precios del ETF y tasa libre de riesgo hasta hoy.
    Reutiliza exactamente la misma logica que main.py para consistencia.
    """
    today = pd.Timestamp.today().strftime("%Y-%m-%d")
    print(f"\n  Descargando precios {ticker} (2008 → {today})...")
    raw = yf.download(ticker, start=DATA_START, end=today, progress=False)
...
def run_walk_forward(ticker: str, run_oos: bool = False) -> dict:
    ...
    Los folds y el OOS se calculan dinámicamente al 70/30 del historial real.
    ...
    # 2. Folds y OOS dinámicos al 70/30
    folds, oos_start, oos_end, val_years = get_folds_and_oos(data, ticker)
    ...
    # 5. Evaluacion OOS (30% final del historial — solo si se pide)
    oos_result = None
    if run_oos:
        print(f"\n  EVALUACION OOS ({oos_start} → {oos_end}) con params optimos...")
        oos_result = run_one(
            data, rf,
            start = oos_start,
            end   = oos_end,
    ...
    if oos_result:
        resumen_rows.append({
            "fold":              f"OOS ({oos_start[:7]} → {oos_end[:7]})",
            "val_start":         oos_start,
            "val_end":           oos_end,
    ...
    params_output = {
        ...
        "esquema_wf":        f"70% train+val ({data.index[0].date()} → {oos_start}) | 30% OOS ({oos_start} → {oos_end})",
        "n_folds":           len(folds),
        "oos_start":         oos_start,
        "oos_end":           oos_end,
        ...
        "nota": (
            "Para usar estos parametros en main_momentum.py, "
            "actualiza WF_LOOKBACK, WF_SKIP y WF_ALPHA_MIN con los valores de params_optimos"
        )
    }
...
def generar_graficas(df_all: pd.DataFrame, df_resumen: pd.DataFrame,
                     ...
                     oos_start: str = "OOS", oos_end: str = "") -> None:
    """
...
def generar_tabla_resumen(resumen_por_ticker: dict, output_dir: Path) -> None:
    ...
    _palette = ["#2E75B6", "#70AD47", "#FF9900", "#C00000", "#7030A0", "#00B0F0", "#FF6699"]
    ticker_colors_map = {t: _palette[i % len(_palette)] for i, t in enumerate(tickers)}
...
def generar_grafica_comparativa(resumen_por_ticker: dict, output_dir: Path) -> None:
    ...
    _palette = ["#2E75B6", "#70AD47", "#FF9900", "#C00000", "#7030A0", "#00B0F0", "#FF6699"]
    ticker_colors = {t: _palette[i % len(_palette)] for i, t in enumerate(tickers)}
...
def main():
    ...
    for ticker in tickers:
        ...
        generar_graficas(
            ...
            oos_start      = resultado.get("oos_start", "OOS"),
            oos_end        = resultado.get("oos_end", ""),
        )
