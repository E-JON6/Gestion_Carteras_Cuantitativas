...
walk_forward_e4.py
==================
Walk-Forward Analysis para optimizar los parámetros de E4_DN_Defensive.
...
Grid de busqueda (18 combinaciones x 4 folds = 72 backtests):
    sigma_target    : [0.10, 0.15, 0.20]  volatilidad objetivo (10%, 15%, 20%)
    trend_window    : [150, 200, 250]      ventana SMA tendencia (dias)
    vol_crisis_mult : [1.5, 2.0]          multiplicador umbral crisis

Criterio de seleccion (score combinado):
    40% Sharpe medio + 30% Calmar medio + 20% Consistencia + 10% (-Volatilidad)
    Todo normalizado a [0,1] antes de ponderar.

Uso:
    # Desde la carpeta raiz del proyecto (donde esta main.py):
    python walk_forward_e4.py --ticker MSE.PA
    python walk_forward_e4.py --ticker MSE.PA --oos
    python walk_forward_e4.py --oos   # todos los ETFs

Outputs (carpeta resultados/WALK_FORWARD_E4/):
    wf_e4_resultados_TICKER.csv   -> Sharpe de cada combinacion en cada fold
    wf_e4_resumen_TICKER.csv      -> Parametros ganadores por fold + OOS final
    wf_e4_params_optimos.json     -> Parametros finales listos para usar
"""
...
PARAM_GRID = {
    "sigma_target":    [0.10, 0.15, 0.20],
    "trend_window":    [150, 200, 250],
    "vol_crisis_mult": [1.5, 2.0],
}

# Los 4 ETFs del proyecto
ALL_TICKERS = ["MSE.PA", "IUSE.L", "IEMA.L", "IUSN.DE"]

OUTPUT_DIR = PROJECT_ROOT / "resultados" / "WALK_FORWARD_E4"
...
def run_one(data: pd.DataFrame, rf: pd.Series,
            start: str, end: str,
            st: float, tw: int, vcm: float,
            lam: float) -> dict:
    """
    Ejecuta BacktestEngine con E4_DN_Defensive para una combinacion de params.
...
def run_walk_forward(ticker: str, run_oos: bool = False) -> dict:
    ...
    """
    print("\n" + "=" * 65)
    print(f"  WALK-FORWARD E4 DEFENSIVO — {ticker}")
    ...
    for fold in folds:
        ...
        for combo in combos:
            ...
            res    = run_one(
                ...
                st    = params["sigma_target"],
                tw    = params["trend_window"],
                vcm   = params["vol_crisis_mult"],
                lam   = lam,
            )
            row = {
                "fold":            fold["label"],
                ...
                "sigma_target":    params["sigma_target"],
                "trend_window":    params["trend_window"],
                "vol_crisis_mult": params["vol_crisis_mult"],
        ...
        print(f"    → Mejor: sigma_target={best_params['sigma_target']}  "
              f"trend_window={best_params['trend_window']}d  "
              f"vol_crisis_mult={best_params['vol_crisis_mult']}  "
    ...
    keys_combo = ["sigma_target", "trend_window", "vol_crisis_mult"]
    ...
    params_optimos = {
        "sigma_target":    float(best_row["sigma_target"]),
        "trend_window":    int(best_row["trend_window"]),
        "vol_crisis_mult": float(best_row["vol_crisis_mult"]),
    }

    # Imprimir top 5 para transparencia
    print(f"\n  TOP 5 COMBINACIONES POR SCORE COMBINADO:")
    print(f"  {'ST':>5} {'TW':>4} {'VCM':>5}  {'Sharpe':>7} {'Calmar':>7} "
          f"{'Consist':>8} {'Score':>7}")
    print(f"  {'-'*55}")
    for _, r in agg.nlargest(5, "score").iterrows():
        print(f"  {r['sigma_target']:>5.2f} {int(r['trend_window']):>4}d "
              f"{r['vol_crisis_mult']:>5.1f}  "
              f"{r['sharpe_medio']:>7.3f} {r['calmar_medio']:>7.3f} "
              f"{r['consistencia']*100:>7.0f}%  {r['score']:>7.4f}")

    print(f"\n  PARAMETROS OPTIMOS (score combinado):")
    print(f"    sigma_target    = {params_optimos['sigma_target']:.2f}  ({params_optimos['sigma_target']*100:.0f}% vol objetivo)")
    print(f"    trend_window    = {params_optimos['trend_window']} dias  (SMA tendencia)")
    print(f"    vol_crisis_mult = {params_optimos['vol_crisis_mult']:.1f}  (umbral crisis = {params_optimos['vol_crisis_mult']*params_optimos['sigma_target']*100:.0f}%)")
    ...
    if run_oos:
        ...
        oos_result = run_one(
            ...
            st    = params_optimos["sigma_target"],
            tw    = params_optimos["trend_window"],
            vcm   = params_optimos["vol_crisis_mult"],
            lam   = lam,
        )
    ...
    if oos_result:
        resumen_rows.append({
            ...
            "sigma_target":    params_optimos["sigma_target"],
            "trend_window":    params_optimos["trend_window"],
            "vol_crisis_mult": params_optimos["vol_crisis_mult"],
        })
    ...
    params_output = {
        ...
        "nota": (
            "Para usar estos parametros en main_e4.py, "
            "pasa sigma_target, trend_window y vol_crisis_mult al constructor "
            "de E4_DN_Defensive, o actualiza E4_VOL_CRISIS_MULT, E4_TREND_WINDOW "
            "y SIGMA_TARGET en config.py"
        )
    }
    ...
    mask_opt = (
        (df_opt["sigma_target"]    == params_optimos["sigma_target"])    &
        (df_opt["trend_window"]    == params_optimos["trend_window"])     &
        (df_opt["vol_crisis_mult"] == params_optimos["vol_crisis_mult"])
    )
...
def _label_combo(st, tw, vcm):
    """Etiqueta corta para una combinacion de parametros."""
    return f"ST{int(st*100)}%-TW{tw}d-VCM{vcm:.1f}"
...
def generar_graficas(df_all: pd.DataFrame, df_resumen: pd.DataFrame,
    ...
    Graficas:
        1. wf_e4_heatmap_TICKER.png       — Sharpe de cada combo por fold
        2. wf_e4_estabilidad_TICKER.png   — Params ganadores por fold
        3. wf_e4_sensibilidad_TICKER.png  — Sharpe medio al variar cada param
        4. wf_e4_degradacion_TICKER.png   — Sharpe IS → Val media → OOS
    """
    output_dir.mkdir(parents=True, exist_ok=True)
    t = ticker.replace(".", "_")
    ...
    df_all["combo"] = df_all.apply(
        lambda r: _label_combo(r["sigma_target"],
                               r["trend_window"],
                               r["vol_crisis_mult"]), axis=1
    )
    ...
    opt_label = _label_combo(params_optimos["sigma_target"],
                              params_optimos["trend_window"],
                              params_optimos["vol_crisis_mult"])
    ...
    ax.set_title(
        f"Heatmap Sharpe — Walk-Forward E4 DN+Defensivo ({ticker})\n"
        f"Recuadro azul = parámetros óptimos seleccionados",
        fontsize=11, fontweight="bold", pad=12
    )
    ax.set_xlabel("Fold de validación", fontsize=10)
    ax.set_ylabel("Combinación de parámetros  (ST=sigma_target  TW=trend_window  VCM=vol_crisis_mult)",
                  fontsize=9)
    fig.tight_layout()
    path1 = output_dir / f"wf_e4_heatmap_{t}.png"
    ...
    params_plot = [
        ("sigma_target", "Sigma Target", [0.10, 0.15, 0.20]),
        ("trend_window",    "Trend Window (dias)", [150, 200, 250]),
        ("vol_crisis_mult", "Vol Crisis Mult",  [1.5, 2.0]),
    ]
    ...
    fig.tight_layout()
    path2 = output_dir / f"wf_e4_estabilidad_{t}.png"
    ...
    fig.tight_layout()
    path3 = output_dir / f"wf_e4_sensibilidad_{t}.png"
    ...
    best_combos   = [
        f"ST{r['sigma_target']*100:.0f}%/TW{int(r['trend_window'])}d/VCM{r['vol_crisis_mult']:.1f}"
        for _, r in best_df.iterrows()
    ]

    # Datos Serie B — combo óptimo final en cada fold
    opt_st = params_optimos["sigma_target"]
    opt_tw = params_optimos["trend_window"]
    opt_vcm = params_optimos["vol_crisis_mult"]
    opt_label = f"Óptimo final: ST{opt_st*100:.0f}%/TW{opt_tw}d/VCM{opt_vcm:.1f}"

    sharpes_opt = []
    for fold_lbl in best_df["fold"].tolist():
        mask = (
            (df_all["fold"]               == fold_lbl) &
            (df_all["sigma_target"]    == opt_st)   &
            (df_all["trend_window"]    == opt_tw)   &
            (df_all["vol_crisis_mult"] == opt_vcm)
        )
        sub = df_all[mask]
    ...
    ax.set_title(
        f"Degradación Temporal — Walk-Forward E4 DN+Defensivo ({ticker})\n"
        f"Azul = mejor combo posible en cada fold  |  "
        f"Naranja = combo óptimo final (ST{opt_st*100:.0f}%/TW{opt_tw}d/VCM{opt_vcm:.1f}) aplicado a todos",
        fontsize=10, fontweight="bold", pad=14
    )
    fig.tight_layout()
    path4 = output_dir / f"wf_e4_degradacion_{t}.png"
...
def generar_tabla_resumen(resumen_por_ticker: dict, output_dir: Path) -> None:
    ...
    for t in tickers:
        ...
        fila_txt   += [f"{p['sigma_target']*100:.0f}%",
                       f"{int(p['trend_window'])}d",
                       f"{p['vol_crisis_mult']:.1f}"]
    ...
    ax.set_title(
        f"Tabla Resumen Walk-Forward — E4 DN+Defensivo\n"
...
def generar_grafica_comparativa(resumen_por_ticker: dict, output_dir: Path) -> None:
    ...
    params_plot = [
        ("sigma_target",    "Sigma Target óptimo",    [0.10, 0.15, 0.20]),
        ("trend_window",    "Trend Window óptimo (días)", [150, 200, 250]),
        ("vol_crisis_mult", "Vol Crisis Mult óptimo",  [1.5, 2.0]),
    ]
    ...
    fig.tight_layout()
    path = output_dir / "wf_e4_comparativa_etfs.png"
    fig.savefig(path, dpi=150, bbox_inches="tight")
    plt.close(fig)
...
def main():
    parser = argparse.ArgumentParser(
        description="Walk-Forward Analysis para E4_DN_Defensive — 4 ETFs"
    )
    parser.add_argument(
    ...
    print("=" * 65)
    print(f"  {'ETF':<12} {'SigmaTarget':>12} {'TrendWindow':>12} {'VolCrisisMult':>14} {'Sharpe Val':>12}")
    print(f"  {'-'*12} {'-'*12} {'-'*12} {'-'*14} {'-'*12}")
    for ticker, res in resumen_por_ticker.items():
        p = res["params_optimos"]
        print(f"  {ticker:<12} {p['sigma_target']:>10.2f} "
              f"{p['trend_window']:>6} {p['vol_crisis_mult']:>10.1f} "
