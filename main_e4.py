"""
Script principal: backtesting de la estrategia E4 DN+Defensivo
optimizada mediante Walk-Forward Analysis.

Parámetros óptimos obtenidos por WF (score combinado Sharpe+Calmar+Consistencia):
    sigma_target    = 0.15  (volatilidad objetivo — actualizar con resultados WF)
    trend_window    = 200   (ventana SMA tendencia — actualizar con resultados WF)
    vol_crisis_mult = 2.0   (umbral crisis — actualizar con resultados WF)

Estrategias comparadas:
    - Buy & Hold   : referencia pasiva (100% en el ETF)
    - Benchmark    : Merton puro (rebalanceo semanal sin bandas DN)
    - E4 Defensivo : Davis-Norman + vol targeting condicional con parámetros óptimos WF
...
Resultados guardados en: resultados/e4_strategy/

Uso:
    python -m gestion_cuantitativa.main_e4
"""
...
WF_SIGMA_TARGET    = 0.15  # pendiente de resultados WF
WF_TREND_WINDOW    = 250
WF_VOL_CRISIS_MULT = 2.0
...
def run_backtest_for_etf(ticker, etf_config, base_output_dir,
    ...
    strategies = [
        ...
        BenchmarkMerton(),
        E4_DN_Defensive(
            sigma_target    = WF_SIGMA_TARGET,
            trend_window    = WF_TREND_WINDOW,
            vol_crisis_mult = WF_VOL_CRISIS_MULT,
        ),
    ]
    ...
    print(f"  Parámetros E4: sigma_target={WF_SIGMA_TARGET:.0%} / "
          f"trend_window={WF_TREND_WINDOW}d / vol_crisis_mult={WF_VOL_CRISIS_MULT:.1f}")
    print("-" * 70)

    m_buyhold   = compute_metrics(results[0], avg_risk_free=avg_rf)
    m_benchmark = compute_metrics(results[1], avg_risk_free=avg_rf)
    m_e4        = compute_metrics(results[2], avg_risk_free=avg_rf)

    # E4 vs Buy & Hold
    print(f"\n  1) E4 Defensivo vs Buy & Hold:")
    print(f"     CAGR  : {m_buyhold['CAGR (%)']:+.2f}%  →  {m_e4['CAGR (%)']:+.2f}%  "
          f"({m_e4['CAGR (%)'] - m_buyhold['CAGR (%)']:+.2f}%)")
    print(f"     Sharpe: {m_buyhold['Sharpe Ratio']:.3f}  →  {m_e4['Sharpe Ratio']:.3f}  "
          f"({m_e4['Sharpe Ratio'] - m_buyhold['Sharpe Ratio']:+.3f})")
    print(f"     MaxDD : {m_buyhold['Max Drawdown (%)']:.1f}%  →  {m_e4['Max Drawdown (%)']:.1f}%  "
          f"({m_e4['Max Drawdown (%)'] - m_buyhold['Max Drawdown (%)']:+.1f}%)")
    print(f"     Calmar: {m_buyhold['Calmar Ratio']:.3f}  →  {m_e4['Calmar Ratio']:.3f}")
    print(f"     Costes: {m_buyhold['Costes (% patrimonio)']:.2f}%  →  "
          f"{m_e4['Costes (% patrimonio)']:.2f}%")

    # E4 vs Benchmark Merton
    print(f"\n  2) E4 Defensivo vs Benchmark Merton:")
    print(f"     CAGR  : {m_benchmark['CAGR (%)']:+.2f}%  →  {m_e4['CAGR (%)']:+.2f}%  "
          f"({m_e4['CAGR (%)'] - m_benchmark['CAGR (%)']:+.2f}%)")
    print(f"     Sharpe: {m_benchmark['Sharpe Ratio']:.3f}  →  {m_e4['Sharpe Ratio']:.3f}  "
          f"({m_e4['Sharpe Ratio'] - m_benchmark['Sharpe Ratio']:+.3f})")
    print(f"     MaxDD : {m_benchmark['Max Drawdown (%)']:.1f}%  →  {m_e4['Max Drawdown (%)']:.1f}%  "
          f"({m_e4['Max Drawdown (%)'] - m_benchmark['Max Drawdown (%)']:+.1f}%)")
    print(f"     Calmar: {m_benchmark['Calmar Ratio']:.3f}  →  {m_e4['Calmar Ratio']:.3f}")
    print(f"     Trades: {m_benchmark['Nº Operaciones']}  →  {m_e4['Nº Operaciones']}  "
          f"({m_e4['Nº Operaciones'] - m_benchmark['Nº Operaciones']:+d})")
    print(f"     Costes: {m_benchmark['Costes (% patrimonio)']:.2f}%  →  "
          f"{m_e4['Costes (% patrimonio)']:.2f}%  "
          f"({m_e4['Costes (% patrimonio)'] - m_benchmark['Costes (% patrimonio)']:+.2f}%)")

    # Ranking
    print(f"\n  RANKING {period_label} ({etf_name}) — por Sharpe Ratio:")
    estrategias_ranked = sorted(
        [(m_e4,        f"E4 DN+Defensivo (ST={WF_SIGMA_TARGET:.0%}/TW={WF_TREND_WINDOW}d/VCM={WF_VOL_CRISIS_MULT:.1f})"),
...
def main():
    print("=" * 70)
    print("  BACKTESTING E4 DN+DEFENSIVO — Parámetros optimizados por Walk-Forward")
    print("  Máster en Finanzas Cuantitativas — AFI")
    print(f"  Parámetros WF: sigma_target={WF_SIGMA_TARGET:.0%} / "
          f"trend_window={WF_TREND_WINDOW}d / vol_crisis_mult={WF_VOL_CRISIS_MULT:.1f}")
